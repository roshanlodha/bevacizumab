...
todo: \* add SOX10 images \* look into EGR1 expression in blood cancers \* look into 3D vasculature as a predictor of response to bevacizumab \* look into IDH1 data
...
#collagen <- collagen[order(collagen$log.expr), ]

collagen_plot <- ggplot(data = collagen, mapping = aes(x = sample, y = gene, fill = log.expr)) +
  geom_tile() +
  #scale_y_discrete(limits=(collagen$gene)[order(collagen$log.expr)]) +
...
deseq.gsea.res <- GSEA(deseq.gsea, pvalueCutoff = 1, TERM2GENE = hs_gsea_tcga_gbm, verbose = FALSE)
...
```{r eg1-poor-vs-good-ihc, fig.width=}
...
## EGR1 Nuclear Expression

```{r egr1-nuclear-expression-plot}
egr1.nuc.plot <- ggplot(egr1_nuclear_expr, aes(x = fct_rev(bev_resp), y = egr1_integrated_density)) +
  geom_boxplot(alpha = 0.5, fill = c("#ffb464", "#126079")) +
  ...
  guides(fill = guide_legend(title = "Animal ID")) +
  #stat_compare_means(label.x.npc = "left", label.y.npc = "bottom") +
  ggtitle("B. EGR1 Expression") +
  xlab("Bevacizumab Response Group") + ylab("Nuclear Fluorescence Intensity (Integrated Density)") + 
  guides(fill = guide_legend(nrow = 5))
```
...
a7.nAChR.plot <- ggplot(nachra7_expr, aes(x = fct_rev(bev_resp), y = nachra7_integrated_density)) +
  ...
  guides(fill = guide_legend(title = "Animal ID")) +
  #stat_compare_means(label.x.npc = "left", label.y.npc = "bottom") +
  ggtitle("C. α7-nAChR Expression") +
  xlab("Bevacizumab Response Group") + ylab("Fluorescence Intensity (Integrated Density)")
```

## SOX10 Nuclear Expression

```{r sox10-nuclear-expression-plot}
sox10.nuc.plot.density <- ggplot(sox10_nuclear_expr, aes(x = fct_rev(bev_resp), y = sox10_integrated_density_v2)) +
  ...
  guides(fill = guide_legend(title = "Animal ID")) +
  #stat_compare_means(label.x.npc = "left", label.y.npc = "bottom") +
  ggtitle("A. SOX10 Expression (Nuclear)") +
  xlab("Bevacizumab Response Group") + ylab("Nuclear Fluorescence Intensity (Integrated Density)")

sox10.nuc.plot.count <- ggplot(sox10_nuclear_expr, aes(x = fct_rev(bev_resp), y = number_of_sox10_cells)) +
  geom_boxplot(alpha = 0.5, fill = c("#126079", "#ffb464")) +
  geom_dotplot(aes(fill = factor(sample_id)),
    binaxis = "y",
    stackdir = "center",
  ...
  guides(fill = guide_legend(title = "Animal ID")) +
  #stat_compare_means(label.x.npc = "left", label.y.npc = "bottom") +
  ggtitle("B. SOX10 Expression (Nuclear)") +
  xlab("Bevacizumab Response Group") + ylab("Number of SOX10+ Cells")
```

## Correlation between a7-nAChR and EGR1

```{r a7-nAChR-egr1-corr}
...
combined_expr_table <- merge(x = egr1_nuclear_expr, y = nachra7_expr, by = c("sample_id", "field")) %>%
  select(c("sample_id", "field", "trt.x", "egr1_integrated_density", "nachra7_integrated_density")) #%>%
  #group_by(sample_id) %>% 
  #summarise_at(vars(-trt.x, -field), funs(mean(., na.rm=TRUE)))

egr1.chrna7.corr.plot <- ggplot(combined_expr_table, aes(x = egr1_integrated_density, y = nachra7_integrated_density)) +
  ...
  #stat_compare_means(label.x.npc = "left", label.y.npc = "bottom") +
  ggtitle("D. EGR1 Expression vs α7-nAChR Expression") +
  xlab("EGR1 Expression (Log Integrated Density)") + ylab("α7-nAChR Expression (Log Integrated Density)") +
  geom_smooth(method=lm, se=FALSE, colour="black")
```

```{r resid-plot}
egr1.nachr7.expr.corr <- lm(nachra7_integrated_density ~ egr1_integrated_density, data = combined_expr_table)
resid.plot <- ggplot(egr1.nachr7.expr.corr, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0) +
  theme_prism() +
  ggtitle("E. Linear Model Residuals of EGR1 vs α7-nAChR") +
  xlab("Residuals") + ylab("Filtered")
```

# Figure 3

```{r figure-3, fig.height=10, fig.width=15}
f3 <- ggarrange(ggarrange(egr1.ihc.plot, egr1.nuc.plot, ncol = 2, nrow = 1),
                ggarrange(a7.nAChR.plot, egr1.chrna7.corr.plot, ncol = 1, nrow = 2),
                #ggarrange(egr1.chrna7.corr.plot, resid.plot, ncol = 1, nrow = 2), 
                ncol = 2, nrow = 1)
f3
```

# Figure 4 
```{r figure-4, fig.height=5, fig.width=15}
f4 <- ggarrange(sox10.nuc.plot.density, sox10.nuc.plot.count, NULL, ncol = 3, nrow = 1)
f4
```
# Survival Analysis
...
pkgs <- c("UCSCXenaTools", "dplyr", "survival", "survminer", "ggbreak", "ggprism", "svglite", "tidyverse")
...
CHRNA7 = fetch_dense_values(host = ge$XenaHosts,
  ...
  use_probeMap = TRUE) %>% .[1, ]
```
## TCGA RNA-seqAnalysis

### EGR1 expression angiogenesis correlation
```{r EGR1-angiogenesis-heatmap}
...
EGR1_angiogensis_plot <- ggplot(data = EGR1_angiogensis_pivoted %>% 
  ...
  ggtitle("C. Angiogenesis Genes RNA Expression") +
  facet_grid(~group,
    switch = "x", scales = "free_x", space = "free_x") +
  #labs(color = "Your title here") +
  theme_prism() +
...
### EGR1 collagen correlation
```{r EGR1-collagen-heatmap}
...
EGR_collagen_pivoted <- pivot_longer(data = EGR_collagen, 
                                     ...
                                     names_to = "Gene", 
                                     values_to = "Expression")
EGR_collagen_plot <- ggplot(data = EGR_collagen_pivoted %>% 
                              dplyr::filter(group != "EGR1 normal") %>%
                              dplyr::filter(Gene != "EGR1"), 
                               mapping = aes(x = rowname, y = Gene, fill = Expression)) +
  geom_tile() +
...
EGR1_collagen_angiogenesis <- as.data.frame(EGR1_collagen_angiogenesis_matrix) %>% 
  ...
  mutate(EGR1 = case_when(
    z > 1.5~'EGR1 upregulation',
    z < -1.5~'EGR1 downregulation',
...
EGR1_collagen_angiogenesis_pivoted <- pivot_longer(data = EGR1_collagen_angiogenesis, 
                                                   ...
                                                   names_to = "Gene", 
                                                   values_to = "Expression") %>%
  dplyr::mutate(group = ifelse(Gene %in% goi, "angiogenesis", "collagen"))

EGR1_collagen_angiogensis_plot <- ggplot(EGR1_collagen_angiogenesis_pivoted %>% 
         dplyr::filter(EGR1 != "EGR1 normal"),
       aes(x = group, y = Expression, fill = EGR1)) +            # Applying ggplot function
  geom_boxplot(alpha = 0.5) +
  #geom_dotplot(binaxis='y', stackdir='center') +
  #geom_jitter() +
  #stat_compare_means(test = "t.test") + 
  scale_fill_manual(values = c("#126079", "#ffb464")) + 
  theme_prism() +
  ylab("Log Expression") +
  xlab("Gene Set") + 
  ggtitle("B. Gene Set Expression by EGR1 Expression") +
  theme(legend.position = "bottom")
```

### EGR1 expression survival plot

```{r EGR1-survival, fig.width=10, fig.height=7}
...
fit_EGR1 = survfit(Surv(time, status) ~group,
  data = merged_EGR1 %>% dplyr::filter(group != "Normal"))
EGR1_plot <- ggsurvplot(fit_EGR1,
  ...
  break.time.by = 250,
    ggtheme = theme_prism(),
    legend = c(0.7, 0.8),
    title = "A. EGR1-expression Stratisfied Survival Plot")
...
```{r supplemental-figure-4, fig.height=6.5, fig.width=10}
arrange_ggsurvplots
splots <- list()
...
```{r supplemental-figure-3, fig.height=5, fig.width=10}
...
```{r figure-5, fig.height=10, fig.width=15}
f5 <- ggarrange(EGR1_plot,
                ggarrange(EGR1_collagen_angiogensis_plot, ggarrange(EGR1_angiogensis_plot, EGR_collagen_plot, ncol = 1, nrow = 2), ncol = 2, nrow = 1), 
                ncol = 1, nrow = 2)
f5bcd <- ggarrange(EGR1_collagen_angiogensis_plot, ggarrange(EGR1_angiogensis_plot, EGR_collagen_plot, ncol = 1, nrow = 2), ncol = 2, nrow = 1)
f5bcd
```
