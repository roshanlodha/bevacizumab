...
```{r suppfig3}
suppfig3a <- ggplot(deseq.GSEA.df %>% dplyr::filter(phenotype == "poor")) +
...
suppfig3b <- ggplot(deseq.GSEA.df %>% dplyr::filter(phenotype == "good")) +
  geom_col(aes(x = reorder(Description, -NES), y = NES, fill = NES)) +
  coord_flip() + 
  scale_fill_gradient2(low = "red", mid = "blue", high = "blue", midpoint = 0) +
...
f1 <- ggarrange(ggarrange(fig1a, fig1b, ncol = 1, nrow = 2), 
...
### Figure 2
```{r fig2, fig.width=16, fig.height=6}
f2 <- ggarrange(fig2a, NULL,  fig2c, NULL, nrow = 1, widths = c(1, 2, 1, 1))
f2
```
...
sox10_nuclear_expr <- read_excel("./input/expr.xlsx", sheet = "sox10_nuclear") %>%
  mutate(bev_resp = recode(bev_resp, good_resp = 'Good Response', poor_resp = 'Poor Response'))
suppfig8a <- ggplot(sox10_nuclear_expr, aes(x = fct_rev(bev_resp), y = sox10_integrated_density_v2)) +
...
suppfig8b <- ggplot(sox10_nuclear_expr, aes(x = fct_rev(bev_resp), y = number_of_sox10_cells)) +
  geom_boxplot(alpha = 0.5, fill = c("#126079", "#ffb464")) +
  geom_dotplot(aes(fill = factor(sample_id)),
    binaxis = "y",
    stackdir = "center",
...
```{r suppfig8, fig.height=6, fig.width=8}
suppfig8 <- ggarrange(suppfig8a, suppfig8b, nrow = 1, ncol = 2)
suppfig8
```
...
fig3b <- ggplot(combined_expr_table, aes(x = egr1_mean_intensity, y = nachra7_mean_intensity)) +
  ...
  xlab("EGR1 Intensity") + ylab("α7-nAChR Intensity") +
  stat_cor() + 
...
f3 <- ggarrange(NULL, fig3b, ncol = 1, nrow = 2)
f3
```
...
```{r tcga-egr1-angiogensis-rna-plot}
...
```{r tcga-angiogenesis-collagen-egr1-box-plot}
blood_vessel <- scan("./input/blood_vessel_geneset.txt", character(), quote = "")
...
```{r EGR1-survival, fig.width=10, fig.height=6}
...
```{r suppfig6}
suppfig6 <- ggsurvplot(fit, data=clin_df, 
...
```{r fig5, fig.height=10, fig.width=15}
f5 <- ggarrange(ggarrange(fig5a, fig5b, ncol = 2, nrow = 1, common.legend = TRUE, legend = "none"), 
                ggarrange(fig5c, fig5d, ncol = 2, nrow = 1, widths = c(1, 0.5)),
                ncol = 1, nrow = 2, heights = c(0.6, 1))
f5
```

## Other Cancers
### Pan-Cancer Analysis
```{r pancan, fig.height=10, fig.width=10}
...
suppfig7a <- ggsurvplot(fit_CHRNA7,
  pval = TRUE,
  pval.coord = c(600, 0.5),
  #xlim = c(0, 1000),
  palette = c("#ffb464", "#126079"),
...
suppfig7b <- ggsurvplot(fit_SOX10,
  pval = TRUE,
  pval.coord = c(600, 0.5),
  #xlim = c(0, 1000),
  palette = c("#ffb464", "#126079"),
...
suppfig7d <- ggsurvplot(fit_RAMP3,
  pval = TRUE,
  pval.coord = c(600, 0.5),
  #xlim = c(0, 1000),
  palette = c("#ffb464", "#126079"),
...
```{r suppfig7, fig.height=10, fig.width=6}
splots <- list()
splots[[1]] <- suppfig7a
splots[[2]] <- suppfig7b

sf7 <- arrange_ggsurvplots(splots, print = FALSE, ncol = 1, nrow = 2, risk.table.height = 0.35)
sf7
```